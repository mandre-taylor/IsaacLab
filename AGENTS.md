...
- **Unit tests may only import packages declared in the package's `config/extension.toml`** (under `[python.pipapi].requirements` or `[dependencies]`). If a test imports anything else, one of the following must happen:
  1. The missing package is a genuine dependency → add it to `config/extension.toml` **and** `pyproject.toml` before writing the test.
...
- **Do not silently rely on transitive or ambient packages.** If a unit test needs `scipy`, `torch`, or `warp`, those must be listed as explicit deps in `config/extension.toml`; do not assume they will be present because another package happened to install them.
- The authoritative reference for what "unit-test-safe" imports look like is `source/isaaclab/config/extension.toml`. When in doubt, check that file before adding a new import to a unit test.
